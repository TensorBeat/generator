...
class GetSongsByTagsRequest(betterproto.Message):
    # Pass in map of tags to be matched on returned songs  EX:  {  "genre":
    # "rock",  "spectrogram_id": "*",  }  The tags will be combined using the
    # filter:  - ANY   means songs matching any of the tags will be returned.  -
    # ALL   means songs matching all of the tags will be returned.  - NONE  means
    # songs that dont match any of the tags will be returned.   Using an * for
    # the value will return any song with that tag set.  Using a specific value
    # for the tag will return only songs with that exact combination of Key/Value
    tags: Dict[str, str] = betterproto.map_field(
        1, betterproto.TYPE_STRING, betterproto.TYPE_STRING
    )
    filter: "Filter" = betterproto.enum_field(2)
    page_token: int = betterproto.int64_field(3, group="_page_token")
    page_size: int = betterproto.int64_field(4, group="_page_size")
...
class GetAllSongsRequest(betterproto.Message):
    page_token: int = betterproto.int64_field(1, group="_page_token")
    page_size: int = betterproto.int64_field(2, group="_page_size")
...
class GetSongsByIDsRequest(betterproto.Message):
    ...
    page_token: int = betterproto.int64_field(2, group="_page_token")
    page_size: int = betterproto.int64_field(3, group="_page_size")
...
class DatalakeServiceStub(betterproto.ServiceStub):
    async def get_all_songs(
        self, *, page_token: int = 0, page_size: int = 0
    ) -> "GetAllSongsResponse":
...
class DatalakeServiceBase(ServiceBase):
    async def get_all_songs(
        self, page_token: int, page_size: int
    ) -> "GetAllSongsResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)

    async def get_songs_by_i_ds(
        self, ids: Optional[List[str]], page_token: int, page_size: int
    ) -> "GetSongsByIDsResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)

    async def get_songs_by_tags(
        self, tags: Dict[str, str], filter: "Filter", page_token: int, page_size: int
    ) -> "GetSongsByTagsResponse":
        raise grpclib.GRPCError(grpclib.const.Status.UNIMPLEMENTED)
